...
router.post('/send-verification-email', (req, res) => {
  ...
  db.query('SELECT * FROM users WHERE email = ?', [email], (err, users) => {
    ...
    db.query(
      ...
      (err) => {
        if (err) {
          console.error('Error deleting previous verification codes:', err);
          return res.status(500).json({
            success: false,
            message: '기존 인증 코드 삭제 중 오류가 발생했습니다.',
          });
        }

        // 새 인증 코드 저장
        db.query(
          'INSERT INTO email_verification (email, verification_code, expires_at, is_verified) VALUES (?, ?, ?, FALSE)',
      ...
      db.query(
        ...
        (err) => {
          if (err) {
            ...
            return res.status(500).json({
              success: false,
              message: '인증 상태 업데이트 중 오류가 발생했습니다.',
            });
          }
